...
```{r, use_correlation_last_value,webgl=TRUE,results='hide'}
...
```{r, correlation_summary_last} 

tbl_summary(
    last_value_df %>% ungroup() %>%  select(-PATIENT_ID, -color, -age, -gender),
    by = outcome
) %>%
    add_n() %>%
